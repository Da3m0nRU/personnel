...
class AddAbsenceDialog(ctk.CTkToplevel):
    ...
    def create_widgets(self):
        ...
        # --- Сотрудник ---
        ctk.CTkLabel(dialog_frame, text="Сотрудник*", font=DEFAULT_FONT).pack(anchor="w")
        ...
        ctk.CTkLabel(dialog_frame, text="Дата отсутствия*", font=DEFAULT_FONT).pack(anchor="w")
        date_frame = ctk.CTkFrame(dialog_frame, fg_color="transparent")
        date_frame.pack(fill="x", pady=(2, 10))

        # День
        ctk.CTkLabel(date_frame, text="День", font=DEFAULT_FONT).grid(row=0, column=0, padx=(0,5), sticky="w")
        ...
        # Месяц
        ctk.CTkLabel(date_frame, text="Месяц", font=DEFAULT_FONT).grid(row=0, column=1, padx=(0,5), sticky="w")
        ...
        # Год
        ctk.CTkLabel(date_frame, text="Год", font=DEFAULT_FONT).grid(row=0, column=2, padx=(0,5), sticky="w")
        ...
        ctk.CTkLabel(self.time_frame, text="Начало (ЧЧ:ММ)", font=DEFAULT_FONT).grid(row=0, column=0, padx=(0,10), sticky="w")
        self.start_time_entry = ctk.CTkEntry(self.time_frame, font=DEFAULT_FONT, width=80)
        self.start_time_entry.grid(row=1, column=0, padx=(0,10), sticky="w")
        self.start_time_entry.bind("<KeyRelease>", self.check_fields) # Проверка при вводе

        ctk.CTkLabel(self.time_frame, text="Окончание (ЧЧ:ММ)", font=DEFAULT_FONT).grid(row=0, column=1, sticky="w")
        self.end_time_entry = ctk.CTkEntry(self.time_frame, font=DEFAULT_FONT, width=80)
        self.end_time_entry.grid(row=1, column=1, sticky="w")
        self.end_time_entry.bind("<KeyRelease>", self.check_fields) # Проверка при вводе

        # Информационная метка о рабочих часах (пока скрыта)
        self.work_hours_label = ctk.CTkLabel(self.time_frame, text="", font=("Arial", 12), text_color="grey")
        self.work_hours_label.grid(row=2, column=0, columnspan=2, pady=(5, 0), sticky="w")

        # --- Причина ---
        ctk.CTkLabel(dialog_frame, text="Причина*", font=DEFAULT_FONT).pack(anchor="w")
        self.reason_textbox = ctk.CTkTextbox(dialog_frame, font=DEFAULT_FONT, height=100) # Увеличим высоту
        self.reason_textbox.pack(anchor="w", fill="x", pady=(2, 20))
        self.reason_textbox.bind("<KeyRelease>", self.check_fields) # Проверка при вводе
    ...
    def update_schedule_info(self):
         ...
         if not (self._selected_personnel_number and selected_date):
             log.debug("Сотрудник или дата не выбраны/некорректны, информация о графике не запрашивается.")
             return

         log.debug(f"Запрос графика для: Сотрудник={self._selected_personnel_number}, Дата={selected_date}")
         try:
            # Получаем ID должности выбранного сотрудника
            position_id = self.repository.get_employee_position_id(self._selected_personnel_number)
            if not position_id:
                log.warning(f"Не удалось получить PositionID для {self._selected_personnel_number}")
                self.work_hours_label.configure(text="Не удалось определить должность.")
                return
            ...
            schedule_data = self.repository.get_working_hours(position_id, day_of_week_id)
            ...
            if schedule_data:
                ...
                if start_time == "00:00" and end_time == "00:00":
                    log.info(f"Выбран нерабочий день ({selected_date.strftime('%A')}) по графику.")
                    self.work_hours_label.configure(text="Выходной по графику.")
                    ...
                    self.work_hours_label.configure(text=f"График на этот день: {start_time} - {end_time}")
                    log.debug(f"График найден: ScheduleID={self._current_schedule_id}, Часы={start_time}-{end_time}")
            else:
                log.warning(f"График для PositionID={position_id} на {selected_date} ({day_of_week_id}) не найден.")
                self.work_hours_label.configure(text="График работы не найден.")
    ...
    def apply_schedule_to_time_fields(self):
        ...
        if is_full_day:
             ...
             if self._current_working_start_time and self._current_working_end_time:
                 ...
                 self.start_time_entry.insert(0, self._current_working_start_time)
                 ...
                 self.start_time_entry.delete(0, "end")
                 self.end_time_entry.delete(0, "end")
                 log.debug("Поля времени очищены (Полный день, нет графика или выходной).")
        else:
    ...
    def load_employee_list(self):
        ...
        try:
            ...
            if not employee_list:
                 ...
                 messagebox.showwarning("Предупреждение", "Список работающих сотрудников пуст. Невозможно добавить отсутствие.")
                 # Состояние кнопки обновится в check_fields
            else:
                log.debug(f"Загружено {len(employee_list)} сотрудников.")
        except Exception as e:
            log.exception("Ошибка при загрузке списка сотрудников в AddAbsenceDialog")
            messagebox.showerror("Ошибка", "Не удалось загрузить список сотрудников.")


    def toggle_time_fields(self):
        """ Переключает состояние полей времени при клике на чекбокс 'Полный день'. """
        log.debug(f"Сработало toggle_time_fields, FullDay={self.full_day_var.get()}")
    ...
    def check_fields(self, event=None):
         ...
         employee_selected = bool(self.employee_combo.get() and self.get_selected_personnel_number())
         date_selected = bool(self.get_selected_date())
         reason_filled = bool(self.reason_textbox.get("1.0", "end-1c").strip())
         time_is_valid = False # Валидно ли время (либо задано для полного дня, либо введено для неполного)

         is_full_day = self.full_day_var.get() == 1
         if is_full_day:
             # Для полного дня время валидно, если график найден и он рабочий
             time_is_valid = bool(self._current_working_start_time and self._current_working_end_time)
         else:
             # Для неполного дня время валидно, если оба поля заполнены (формат проверит валидация)
             time_is_valid = bool(self.start_time_entry.get().strip() and self.end_time_entry.get().strip())
    ...
    def validate_input(self):
        ...
        if not selected_date:
             messagebox.showerror("Ошибка валидации", "Некорректно выбрана или указана дата.")
             return False, None
        ...
        if not reason:
            messagebox.showerror("Ошибка валидации", "Не указана причина отсутствия.")
            return False, None
        if len(reason) > 200:
             messagebox.showerror("Ошибка валидации", "Причина отсутствия слишком длинная (макс. 200 символов).")
             return False, None
        ...
        if full_day == 1: # Если выбран "Полный день"
            ...
            if self._current_working_start_time and self._current_working_end_time:
                start_time_str = self._current_working_start_time # Берем из сохраненного состояния
                end_time_str = self._current_working_end_time
                final_schedule_id = self._current_schedule_id # Используем ID найденного графика
                log.debug(f"Валидация 'Полный день': Используется ScheduleID={final_schedule_id}, Время {start_time_str}-{end_time_str}")
            else:
                # График не был найден или это был выходной
                messagebox.showerror("Ошибка валидации", "Для полного дня не удалось определить рабочие часы.\nВозможно, это выходной или график не настроен.")
                return False, None
            ...
            if not (start_time_str and end_time_str):
                 messagebox.showerror("Ошибка валидации", "Для неполного дня укажите время начала и окончания.")
                 return False, None
            ...
            if not re.match(time_pattern, start_time_str) or not re.match(time_pattern, end_time_str):
                messagebox.showerror("Ошибка валидации", "Неверный формат времени (ожидается ЧЧ:ММ).")
                return False, None

            # Проверка порядка времени и вхождения в график
            try:
                absence_start = datetime.datetime.strptime(start_time_str, "%H:%M").time()
                absence_end = datetime.datetime.strptime(end_time_str, "%H:%M").time()
                ...
                if self._current_working_start_time and self._current_working_end_time:
                    work_start = datetime.datetime.strptime(self._current_working_start_time, "%H:%M").time()
                    work_end = datetime.datetime.strptime(self._current_working_end_time, "%H:%M").time()
                    ...
                    if not (work_start <= absence_start < work_end and work_start < absence_end <= work_end):
                        ...
                        return False, None
                    log.debug("Валидация 'Неполный день': Время входит в рабочий график.")
                else:
                    ...
                    messagebox.showerror("Ошибка валидации", "Невозможно проверить введенное время, так как график работы на этот день не определен.")
                    return False, None

                # При ручном вводе ID графика не устанавливается
                final_schedule_id = None
                log.debug("Валидация 'Неполный день': Время введено вручную, ScheduleID=None.")

            except ValueError as e:
                 log.error(f"Ошибка сравнения/парсинга времени: {e}")
                 messagebox.showerror("Ошибка валидации", "Ошибка при обработке введенного времени.")
                 return False, None
        ...
        validated_data = {
            ...
            "full_day": full_day,
            "start_time": start_time_str, # Для полного дня берется из графика, для неполного - из поля
    ...
    def save_absence(self):
        ...
        if success:
            messagebox.showinfo("Успех", "Запись об отсутствии успешно добавлена.")
            ...
            # Ошибка на уровне БД
            messagebox.showerror("Ошибка", "Не удалось сохранить запись об отсутствии в базе данных.")
            log.error("Ошибка при вызове insert_absence из диалога (вернул False).")
